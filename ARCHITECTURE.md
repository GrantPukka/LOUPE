...
**As built.** Seven endpoints: `/api/schema`, `/api/query`, `/api/histogram`, `/api/sources`,
`/api/tail`, `/api/patterns`, and `/api/top`, plus `/api/health`. All of them call `internal/session`, which is the same
