// Package blaster generates realistic, messy, multi-format log files for
// testing and demoing loupe.
//
// It is deliberately stdlib-only and CGO-free so it cross-compiles anywhere and
// can be run in CI to regenerate fixtures. It is imported by cmd/blaster, which
// is the flag-parsing shell around it, and by `loupe demo`, which is why it is
// a package rather than living inside the command: a single static binary
// cannot shell out to `go run`.
//
...
// Config describes one generation run. The zero value is not useful; see
// Defaults, which is what both the command and `loupe demo` start from.
type Config struct {
	Out      string        // directory to write into
	Scenario string        // steady | incident | deploy-regression | quiet
	Duration time.Duration // span of simulated time
	Rate     float64       // baseline requests per simulated second
	Seed     int64         // same seed gives byte-identical output
	Malform  float64       // fraction of lines that are broken on purpose
	Follow   bool          // write in real time instead of all at once
	Rotate   bool          // also emit rotated .log.1 and .log.2.gz files

	// Report receives the human-readable summary of what was written. A nil
	// Report discards it: callers that render their own output must not have
	// this appear on stdout behind their back, where it would corrupt a piped
	// `loupe demo --print --format ndjson`.
	Report io.Writer
}

func (c Config) report() io.Writer {
	if c.Report == nil {
		return io.Discard
	}
	return c.Report
...
// Defaults returns the standard incident scenario: eighteen minutes of traffic
// across six sources, with a Postgres pool exhaustion two thirds of the way in.
...
// Run generates the log files described by c.
...
func writeAll(c Config, rng *rand.Rand, events []entry) error {
	...
	w := c.report()
	fmt.Fprintf(w, "wrote %d files to %s\n", len(sinks)+3, c.Out)
	for _, r := range m.Files {
		fmt.Fprintf(w, "  %-22s %-16s %6d records  %3d broken\n",
...
func stream(c Config, events []entry, start, end time.Time) error {
	...
	fmt.Fprintf(c.report(), "following into %s, ctrl-c to stop\n", c.Out)
