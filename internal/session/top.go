...
func (s *Session) Top(ctx context.Context, plan Plan, q TopQuery) (TopSet, error) {
	...
	// The same resolver the filter compiler uses, so a facet and a filter agree
	// about which column a name means — and an unknown name gets the same
	// spelling suggestion here as anywhere else, rather than an empty table.
	expr, err := sch.Column(q.Field)
	if err != nil {
		return TopSet{}, err
	}
...
// it is a slice of.
func (s *Session) topTotals(ctx context.Context, plan Plan, expr string, out *TopSet) error {
	row := s.DB.QueryRow(ctx,
		`SELECT count(*),
		        count(*) FILTER (WHERE (`+expr+`) IS NOT NULL),
		        count(DISTINCT `+expr+`)
...
func (s *Session) topValues(ctx context.Context, plan Plan, expr string) ([]TopValue, error) {
	...
	rows, err := s.DB.Query(ctx,
		`SELECT (`+expr+`) AS value, count(*) AS n
		 FROM logs
		 WHERE (`+plan.SQL.Where+`) AND (`+expr+`) IS NOT NULL
		 GROUP BY 1
		 ORDER BY n DESC, 1`, plan.SQL.Args...)
	if err != nil {
